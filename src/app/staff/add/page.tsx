...
import { User, Phone, Building2, Smartphone, Pencil, Trash2, Plus, Save } from "lucide-react";
...
/** 폼/목록에서 다루는 직원 항목 (id는 임시 가능) */
type StaffDraft = Omit<StaffMember, "level"> & { level: number; _tempId?: string };
...
export default function StaffAddPage() {
  const [form, setForm] = useState<StaffDraft>(emptyForm());
  const [list, setList] = useState<StaffDraft[]>([]);
  const [editingId, setEditingId] = useState<string | null>(null);

  const setField = useCallback(<K extends keyof StaffDraft>(key: K, value: StaffDraft[K]) => {
  ...
  const handleAdd = () => {
    ...
    setForm(emptyForm());
    setEditingId(null);
    toast.success("목록에 추가했습니다. 등록 버튼으로 저장하세요.");
  };

  const handleEdit = (item: StaffDraft) => {
    setForm({ ...item });
    setEditingId(item.id);
    toast.success("편집할 수 있게 폼에 불러왔습니다.");
  };
  ...
  const handleRegister = () => {
    const toSend: StaffMember[] = list.map((item, idx) => {
      ...
      return { ...rest, id, level: rest.level || 1, phone } as StaffMember;
    });
    if (toSend.length === 0) {
      ...
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-700">직원 정보</h2>
          </div>
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={form.role}
                  onChange={(e) => setField("role", e.target.value as StaffDraft["role"])}
                  className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:border-primary-400 focus:ring-2 focus:ring-primary-600/20 outline-none"
                >
                  {ROLE_OPTIONS.map((r) => (
